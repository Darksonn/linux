...
macro_rules! impl_has_list_links {
    ($(impl$(<$($implarg:ident),*>)?
       ...
       { self$(.$field:ident)* }
        ...
        // SAFETY: The implementation of `raw_get_list_links` only compiles if the field has the right
        // type.
        unsafe impl$(<$($implarg),*>)? $crate::list::HasListLinks$(<$id>)? for $self $(<$($selfarg),*>)? {
...
macro_rules! impl_has_list_links_self_ptr {
    ($(impl$({$($implarg:tt)*})?
       ...
       { self.$field:ident }
        ...
        // SAFETY: The implementation of `raw_get_list_links` only compiles if the field has the right
        // type.
        unsafe impl$(<$($implarg)*>)? $crate::list::HasSelfPtr<$item_type $(, $id)?> for $self $(<$($selfarg),*>)? {}
...
macro_rules! impl_list_arc_safe {
    (impl$({$($generics:tt)*})? ListArcSafe<$num:literal> for $t:ty { untracked; } $($rest:tt)*) => {
    ...
    (impl$({$($generics:tt)*})? ListArcSafe<$num:literal> for $t:ty { tracked_by $field:ident : $fty:ty; } $($rest:tt)*) => {
        impl$(<$($generics)*>)? ListArcSafe<$num> for $t {
            unsafe fn on_create_list_arc_from_unique(&mut self) {
...
macro_rules! impl_list_item {
    (
        impl$({$($generics:tt)*})? ListItem<$num:literal> for $t:ty {
            using ListLinks;
        } $($rest:tt)*
    ) => {
    ...
    (
        ...
        unsafe impl$(<$($generics)*>)? ListItem<$num> for $t {
            unsafe fn prepare_to_insert(me: *const Self) -> *mut ListLinks<$num> {
                ...
                let self_ptr = unsafe { (links_field as *const u8).add(spoff) as *const ::core::cell::UnsafeCell<*const Self> };
