...
impl TransactionInfo {
    ...
    pub(crate) fn report_netlink(&self, ctx: &crate::Context) {
        if let Err(err) = self.report_netlink_inner(ctx) {
            pr_warn!("{}:{} netlink report failed: {err:?}\n", self.from_pid, self.from_tid);
        }
    }

    fn report_netlink_inner(&self, ctx: &crate::Context) -> kernel::error::Result {
        if !Report::has_listeners() {
            return Ok(());
        }
        let mut report = Report::new(GENLMSG_DEFAULT_SIZE, 0, 0, GFP_KERNEL)?;

        report.error(self.reply)?;
